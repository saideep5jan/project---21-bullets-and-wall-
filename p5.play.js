...
p5.Renderer2D.prototype.shapeee = function() {
  var ctx = this.drawingContext;
  var doFill = this._doFill, doStroke = this._doStroke;
  if (doFill && !doStroke) {
    if (ctx.fillStyle === styleEmpty) {
...
p5.prototype.shapeee = function() {
  ...
  if (!this._renderer.isP3D) {
    ...
    this._renderer.shapeee.apply(this._renderer, arguments);
  }
  return this;
};
...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  * placeholder rectangle
  *
  * @property shapeeeColor
  * @type {color}
  */
  this.shapeeeColor = color(127, 127, 127);
  ...
  */
   ...
   * shapeee, but it should be sufficient for broad-phase detection of collision
   * candidates.
   *
  ...
  */
  this.draw = function()
  {
    ...
    {
      var fillColor = this.shapeeeColor;
      if (this.tint) {
...
function Group() {
  ...
  */
  ...
  array.setColorEach = array.setPropertyEach.bind(array, 'shapeeeColor');
...
p5.prototype._warn = function(message) {
  ...
  /**
   * Collision shapeee Base Class
   *
   * We have a set of collision shapeees available that all conform to
   ...
   * shapeee and can be used as a collision point with no changes.
   * Other shapeees should inherit from this and override most methods.
   *
   * @class p5.Collisionshapeee
   * @constructor
   * @param {p5.Vector} [center] (zero if omitted)
   * @param {number} [rotation] (zero if omitted)
   */
  p5.Collisionshapeee = function(center, rotation) {
    /**
     * Transform of this shapeee relative to its parent.  If there is no parent,
    ...
    /**
     * Transform of whatever parent object (probably a sprite) this shapeee is
     * associated with.  If this is a free-floating shapeee, the parent transform
    ...
    /**
     * The center of the collision shapeee in world-space.
     * @property _center
     * @private
     * @type {p5.Vector}
     */
    ...
    /**
     * The center of the collision shapeee in local-space; also, the offset of the
     * collision shapeee's center from its parent sprite's center.
     * @property _offset
     * @type {p5.Vector}
     * @private
     */
     ...
     * Note that this will only be meaningful for shapeees that can rotate,
     ...
     * for shapeees that have dimensions (e.g. not for point colliders)
     * @property _scale
     * @type {p5.Vector}
     * @private
     */
    ...
    Object.defineProperties(this, {
       ...
       * actually modify the collision shapeee's local transform.
       * @property center
       * @type {p5.Vector}
       */
      'center': {
      ...
      /**
       * The center of the collision shapeee in local-space - if this collider is
   ...
   * @param {Sprite} sprite
   * @see p5.Collisionshapeee.prototype.getsDimensionsFromSprite
   */
  p5.Collisionshapeee.prototype.updateFromSprite = function(sprite) {
   ...
   */
  p5.Collisionshapeee.prototype.setParentTransform = function(parent) {
   ...
   * of the shapeee's transforms changes.  The base Collisionshapeee (and PointCollider)
   * only need to recompute the shapeee's center, but other shapeees may need to
   * override this method and do additional recomputation.
   * @method _onTransformChanged
   * @protected
   */
  p5.Collisionshapeee.prototype._onTransformChanged = function() {
  ...
  /**
   * Compute the smallest movement needed to move this collision shapeee out of
   * another collision shapeee.  If the shapeees are not overlapping, returns a
   * zero vector to indicate that no displacement is necessary.
   * @method collide
   * @param {p5.Collisionshapeee} other
   * @return {p5.Vector}
   */
  p5.Collisionshapeee.prototype.collide = function(other) {
    var displacee = this, displacer = other;

    // Compute a displacement vector using the Separating Axis Theorem
    // (Valid only for convex shapeees)
    //
    // If a line (axis) exists on which the two shapeees' orthogonal projections
    // do not overlap, then the shapeees do not overlap.  If the shapeees'
    ...
    // shapeees are centrosymmetric: Circles, ellipses, and rectangles
    // are OK.  This lets us only compare the shapeees' radii to the
    // distance between their centers, even for non-circular shapeees.
    // Other convex shapeees, (triangles, pentagons) will require more
    // complex use of their projections' positions on the axis.
    var deltaOfCenters = p5.Vector.sub(displacer.center, displacee.center);

    // It turns out we only need to check a few axes, defined by the shapeees
    // being checked.  For a polygon, the normal of each face is a possible
    // separating axis.
    var candidateAxes = p5.Collisionshapeee._getCandidateAxesForshapeees(displacee, displacer);
    var axis, deltaOfCentersOnAxis, distanceOfCentersOnAxis;
    for (var i = 0; i < candidateAxes.length; i++) {
      axis = candidateAxes[i];

      // If distance between the shapeee's centers as projected onto the
      // separating axis is larger than the combined radii of the shapeees
      // projected onto the axis, the shapeees do not overlap on this axis.
      ...
      if (overlap <= 0) {
        // These shapeees are separated along this axis.
  ...
  /**
   * Check whether this shapeee overlaps another.
   * @method overlap
   * @param {p5.Collisionshapeee} other
   * @return {boolean}
   */
  p5.Collisionshapeee.prototype.overlap = function(other) {
    var displacement = this.collide(other);
    return displacement.x !== 0 || displacement.y !== 0;
  };

  /**
   * @method _getCanididateAxesForshapeees
   * @private
   * @static
   * @param {p5.Collisionshapeee} shapeee1
   * @param {p5.Collisionshapeee} shapeee2
   * @return {Array.<p5.Vector>}
   */
  p5.Collisionshapeee._getCandidateAxesForshapeees = function(shapeee1, shapeee2) {
    var axes = shapeee1._getCandidateAxes(shapeee2)
      .concat(shapeee2._getCandidateAxes(shapeee1))
      .map(function(axis) {
        if (axis.x === 0 && axis.y === 0) {
          return p5.Collisionshapeee.X_AXIS;
        }
        return axis;
      });
   ...
   * @return {Array.<p5.Vector>}
   */
  p5.Collisionshapeee.prototype._getCandidateAxes = function() {
    return [];
  };

  /**
   * Get this shapeee's radius (half-width of its projection) along the given axis.
   * Override this method in subclasses to implement collision behavior.
   ...
   * @return {number}
   */
  p5.Collisionshapeee.prototype._getRadiusOnAxis = function() {
    return 0;
  };

  /**
   * Get the shapeee's minimum radius on any axis for tunneling checks.
   * @method _getMinRadius
   * @protected
   * @param {p5.Vector} axis
   * @return {number}
   */
  p5.Collisionshapeee.prototype._getMinRadius = function() {
    return 0;
  };
   ...
   * @final
   */
  p5.Collisionshapeee.X_AXIS = new p5.Vector(1, 0);
   ...
   * @final
   */
  p5.Collisionshapeee.Y_AXIS = new p5.Vector(0, 1);
   ...
   * @final
   */
  p5.Collisionshapeee.WORLD_AXES = [
    p5.Collisionshapeee.X_AXIS,
    p5.Collisionshapeee.Y_AXIS
  ];

  /**
   * Get world-space axis-aligned bounds information for this collision shapeee.
   * Used primarily for the quadtree.
   * @method getBoundingBox
   * @return {{top: number, bottom: number, left: number, right: number, width: number, height: number}}
   */
  p5.Collisionshapeee.prototype.getBoundingBox = function() {
    var radiusOnX = this._getRadiusOnAxis(p5.Collisionshapeee.X_AXIS);
    var radiusOnY = this._getRadiusOnAxis(p5.Collisionshapeee.Y_AXIS);
    return {
  ...
  /**
   * A point collision shapeee, used to detect overlap and displacement vectors
   * vs other collision shapeees.
   * @class p5.PointCollider
   * @constructor
   * @extends p5.Collisionshapeee
   * @param {p5.Vector} center
   */
  p5.PointCollider = function(center) {
    p5.Collisionshapeee.call(this, center);
  };
  p5.PointCollider.prototype = Object.create(p5.Collisionshapeee.prototype);
  ...
  p5.PointCollider.createFromSprite = function(sprite, offset) {
    // Create the collision shapeee at the transformed offset
    var shapeee = new p5.PointCollider(offset);
    shapeee.setParentTransform(sprite);
    return shapeee;
  };
  ...
  /**
   * A Circle collision shapeee, used to detect overlap and displacement vectors
   * with other collision shapeees.
  ...
  p5.CircleCollider.prototype = Object.create(p5.Collisionshapeee.prototype);
  ...
  p5.CircleCollider.createFromSprite = function(sprite, offset, radius) {
    ...
    var shapeee = new p5.CircleCollider(
      offset,
      customSize ? radius : 1
    );
    shapeee.getsDimensionsFromSprite = !customSize;
    shapeee.updateFromSprite(sprite);
    return shapeee;
  };
  ...
  p5.CircleCollider.prototype._onTransformChanged = function() {
    p5.Collisionshapeee.prototype._onTransformChanged.call(this);
    this._computeScaledRadius();
  };
  ...
  /**
   * Debug-draw this collision shapeee.
   * @method draw
   * @param {p5} sketch instance to use for drawing
   */
  p5.CircleCollider.prototype.draw = function(sketch) {
    ...
    /**
   * Overrides Collisionshapeee.setParentTransform
   * Update this collider's parent transform, which will in turn adjust its
   * position, rotation and scale in world-space and recompute cached values
   * if necessary.
   * If a Sprite is passed as the 'parent' then a new transform will be computed
  ...
  p5.CircleCollider.prototype._getCandidateAxes = function(other) {
    ...
    // TODO: If we can ask the other shapeee for a list of vertices, then we can
    ...
    if (other instanceof p5.OrientedBoundingBoxCollider || other instanceof p5.AxisAlignedBoundingBoxCollider) {
      ...
      // Generate the set of vertices for the other shapeee
    ...
    // axis through both shapeees' centers.
  ...
  /**
   * An Axis-Aligned Bounding Box (AABB) collision shapeee, used to detect overlap
   * and compute minimum displacement vectors with other collision shapeees.
   *
  ...
  p5.AxisAlignedBoundingBoxCollider.prototype = Object.create(p5.Collisionshapeee.prototype);
  ...
  p5.AxisAlignedBoundingBoxCollider.prototype._getCandidateAxes = function() {
    return p5.Collisionshapeee.WORLD_AXES;
  };
  ...
  /**
   * An Oriented Bounding Box (OBB) collision shapeee, used to detect overlap and
   * compute minimum displacement vectors with other collision shapeees.
  ...
  p5.OrientedBoundingBoxCollider = function(center, width, height, rotation) {
    p5.Collisionshapeee.call(this, center, rotation);
    ...
    /**
     * Cached separating axes this shapeee contributes to a collision.
  ...
  p5.OrientedBoundingBoxCollider.prototype = Object.create(p5.Collisionshapeee.prototype);
